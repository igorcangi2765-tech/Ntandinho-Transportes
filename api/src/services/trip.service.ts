...
export class TripService {
   ...
   */
  static async updateTripStatus(tripId: string, status: 'EM_TRANSITO' | 'CONCLUIDO' | 'CANCELADO') {
    try {
      const updateData: any = { status };
      if (status === 'EM_TRANSITO') updateData.departureTime = new Date();
      if (status === 'CONCLUIDO') updateData.arrivalTime = new Date();
      ...
      if (status === 'CONCLUIDO' && trip.vehicleId && trip.driverId) {
        ...
        await FinanceService.generateInvoiceForTrip({
          tripId: trip.id,
      ...
      if (trip) {
        trip.status = status;
        if (status === 'CONCLUIDO') {
          FinanceService.generateInvoiceForTrip({
            tripId,
            customerId: 'cust_cdm_01',
            subtotal: 350000,
            currency: 'MZN',
